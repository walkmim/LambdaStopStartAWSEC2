...
* start_order
    ...
    Default: "0"
    Objetivo: Determina a ordem de início das instâncias. Aplicável em casos como quando o BD precisa ser iniciado antes da aplicação 
    ou também casos de cluster Windows que precise ter ordem ao ligar para sempre promover node específico como primário.
...
* stop_order
    Valores aceitáveis: Numeral a partir de zero.
    Default: "0"
    Objetivo: Determina a ordem de desligamento das instâncias. Aplicável em casos como quando o BD precisa ser iniciado antes da aplicação 
    ou também casos de cluster Windows que precise ter ordem ao ligar para sempre promover node específico como primário.
    Obs: É necessário atribuir esta tag quando grupo de instâncias precisa de ordem e este grupo sempre terá ordem iniciada em "0"